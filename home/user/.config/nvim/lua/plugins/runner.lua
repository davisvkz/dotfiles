	...
	opts = {
		...
		handlers = {
			python = function(buffer)
				require('runner.handlers.helpers').shell_handler('python3 ' ..
					vim.fn.expand('%'):gsub(' ', '\\ '):gsub("'", "\\'"))(buffer)
			end,
			javascript = function(buffer)
				require('runner.handlers.utils').run_command("npm run 2>&1 | grep '  ' | awk '{$1=$1};1' | sed -n 'p;n'",
					function(output)
						local bins = {}
						...
						local handlers = {
							['Run current file'] = helpers.shell_handler(
								'node ' .. vim.fn.expand('%'):gsub(' ', '\\ '):gsub("'", "\\'")),
			...
			typescript = function(buffer)
				require('runner.handlers.utils').run_command("npm run 2>&1 | grep '  ' | awk '{$1=$1};1' | sed -n 'p;n'",
					function(output)
						...
						local handlers = {
							['Run current file'] = helpers.shell_handler('tsx ' .. vim.fn.expand('%'):gsub(' ', '\\ '):gsub("'", "\\'")),
							['Custom'] = helpers.shell_handler('npm ', true),
						}
			...
			cpp = function(buffer)
				local helpers = require('runner.handlers.helpers')
				local filename = vim.fn.expand('%'):gsub(' ', '\\ '):gsub("'", "\\'")
				local outname = vim.fn.expand('%'):gsub(' ', '\\ '):gsub("'", "\\'"):gsub('%.cpp', '')
				helpers.choice({
					['Build Release and run'] = helpers.shell_handler(string.format('g++ -O2 -DNDEBUG -o %s %s && %s', outname,
						filename, outname)),
					['Build debug and run'] = helpers.shell_handler(string.format('g++ -ggdb -o %s %s && %s', outname, filename,
						outname)),
